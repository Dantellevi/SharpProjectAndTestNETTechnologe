        ...
        public Car GetCar(int id)
        {
            ...
            switch(type)
            {
                case "Truck":
                    {
                        result = new TruckCar();
                        ((TruckCar)result).Capasity = double.Parse(element.Element("Capacity").Value, CultureInfo.GetCultureInfo("en-US"));
                        break;
                    }
                case "Passeger":
                    {
                        result = new PassegerCar();
                        ((PassegerCar)result).Passegers = int.Parse(element.Element("Passegers").Value, CultureInfo.GetCultureInfo("en-US"));
                        break;
                    }
                default:
                    {
